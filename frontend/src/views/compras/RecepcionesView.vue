<template>
  <div>
    <div class="d-flex align-center mb-4">
      <v-icon color="purple" size="36" class="mr-3">mdi-truck-delivery</v-icon>
      <h2 class="text-h4 font-weight-bold mb-0">Recepciones de Compra</h2>
      <v-spacer />
      <v-btn color="primary" prepend-icon="mdi-plus" @click="abrirDialogoNuevo">Nueva Recepción</v-btn>
    </div>
    ...
    <v-card v-if="!loading && recepciones.length === 0" variant="tonal" class="pa-8 text-center">
      <v-icon size="64" color="grey-lighten-1" class="mb-4">mdi-truck-delivery-off</v-icon>
      <h3 class="text-h6 text-medium-emphasis">No se encontraron documentos</h3>
      <p class="text-body-2 text-medium-emphasis mt-1">No hay recepciones de compra registradas.</p>
    </v-card>

    <v-data-table v-else :headers="headers" :items="recepciones" :loading="loading" :items-per-page="15" class="elevation-1">
    ...
    <v-dialog v-model="dialogoNuevo" max-width="800" persistent>
      <v-card>
        <v-card-title class="text-h5">Nueva Recepción</v-card-title>
        <v-card-text>
          ...
          <h4 class="text-h6 mb-2">Artículos a recibir</h4>
          <v-row v-for="(linea, i) in nuevoDocumento.articulos" :key="i" class="mb-2" align="center">
            <v-col cols="5">
              <v-autocomplete v-model="linea.articulo_id" :items="articulos" item-title="nombre" item-value="id" label="Artículo" variant="outlined" density="compact" return-object :loading="cargandoCatalogos" :disabled="cargandoCatalogos" no-data-text="No se encontraron artículos" />
            </v-col>
        ...
        <v-card-actions>
          ...
          <v-btn color="success" :loading="guardando" @click="guardar" :disabled="!puedeGuardar || cargandoCatalogos">Recibir</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const recepciones = ref([]); const proveedores = ref([]); const articulos = ref([]); const almacenes = ref([])
...
const puedeGuardar = computed(() => nuevoDocumento.value.almacen_id && nuevoDocumento.value.articulos.length > 0 && nuevoDocumento.value.articulos.every(l => l.articulo_id && l.cantidad > 0))
...
async function guardar() {
  ...
  try {
    await apiClient.post('/api/v1/transacciones', {
      tipo: 'recepcion_compra', entidad_proveedor_id: nuevoDocumento.value.proveedor_id,
    ...
    snackbar.value = { show: true, mensaje: 'Recepción creada', color: 'success' }
    cerrarDialogoNuevo()
    await cargarDatos()
...
function confirmarCancelar(item) {
  mensajeConfirmacion.value = `¿Cancelar ${item.folio}?`
