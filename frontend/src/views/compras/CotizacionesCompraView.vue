<template>
  <div>
    <div class="d-flex align-center mb-4">
      <v-icon color="purple" size="36" class="mr-3">mdi-file-document-outline</v-icon>
      <h2 class="text-h4 font-weight-bold mb-0">Cotizaciones de Compra</h2>
      <v-spacer />
    ...
    <v-card v-if="!loading && cotizaciones.length === 0" variant="tonal" class="pa-8 text-center">
      ...
      <p class="text-body-2 text-medium-emphasis mt-1">No hay cotizaciones de compra registradas.</p>
    </v-card>
    ...
    <v-data-table v-else :headers="headers" :items="cotizaciones" :loading="loading" :items-per-page="15" class="elevation-1">
      ...
      <template v-slot:item.acciones="{ item }">
        ...
        <v-btn icon="mdi-arrow-right-bold" size="small" variant="text" color="primary" @click="convertir(item, 'orden_compra')" v-if="item.estado === 'confirmado'" title="Convertir a Orden de Compra" />
      </template>
    </v-data-table>

    <v-dialog v-model="dialogoNuevo" max-width="800" persistent>
      <v-card>
        <v-card-title class="text-h5">Nueva Cotización de Compra</v-card-title>
        <v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="guardando" @click="guardar" :disabled="!puedeGuardar || cargandoCatalogos">Guardar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const cotizaciones = ref([]); const proveedores = ref([]); const articulos = ref([]); const almacenes = ref([])
...
async function cargarDatos() {
  ...
  try {
    ...
    cotizaciones.value = r.data || []
  } catch (err) {
    console.error('Error al cargar cotizaciones de compra:', err)
...
async function guardar() {
  ...
  try {
    await apiClient.post('/api/v1/transacciones', {
      tipo: 'cotizacion_compra', entidad_proveedor_id: nuevoDocumento.value.proveedor_id,
    ...
    snackbar.value = { show: true, mensaje: 'Cotización creada', color: 'success' }
    cerrarDialogoNuevo()
    await cargarDatos()
...
function convertir(item, nuevoTipo) {
  ...
  accionPendiente = async () => {
    try {
      ...
      snackbar.value = { show: true, mensaje: 'Convertido', color: 'success' }; await cargarDatos()
    } catch (err) { snackbar.value = { show: true, mensaje: err.response?.data?.error || 'Error', color: 'error' } }
  }
  dialogoConfirmar.value = true
}
