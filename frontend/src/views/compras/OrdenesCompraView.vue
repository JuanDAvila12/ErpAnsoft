<template>
  <div>
    <div class="d-flex align-center mb-4">
      <v-icon color="purple" size="36" class="mr-3">mdi-cart-arrow-down</v-icon>
      <h2 class="text-h4 font-weight-bold mb-0">Órdenes de Compra</h2>
      <v-spacer />
    ...
    <v-card v-if="!loading && ordenes.length === 0" variant="tonal" class="pa-8 text-center">
      ...
      <p class="text-body-2 text-medium-emphasis mt-1">No hay órdenes de compra registradas.</p>
    </v-card>
    ...
    <v-data-table v-else :headers="headers" :items="ordenes" :loading="loading" :items-per-page="15" class="elevation-1">
      ...
      <template v-slot:item.acciones="{ item }">
        ...
        <v-btn icon="mdi-arrow-right-bold" size="small" variant="text" color="primary" @click="convertir(item, 'compra')" v-if="item.estado === 'confirmado'" title="Convertir a Compra" />
      </template>
    </v-data-table>
...
const ordenes = ref([]); const proveedores = ref([]); const articulos = ref([]); const almacenes = ref([])
...
async function guardar() {
  ...
  try {
    await apiClient.post('/api/v1/transacciones', {
      tipo: 'orden_compra', entidad_proveedor_id: nuevoDocumento.value.proveedor_id,
    ...
    snackbar.value = { show: true, mensaje: 'Orden creada', color: 'success' }
    cerrarDialogoNuevo()
    await cargarDatos()
