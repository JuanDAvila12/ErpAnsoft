<template>
  <div>
    <div class="d-flex align-center mb-4">
      ...
      <v-spacer />
      <v-btn color="primary" prepend-icon="mdi-plus" @click="abrirDialogoNuevo">Nueva Venta</v-btn>
    </div>
    ...
    <v-data-table v-else :headers="headers" :items="facturas" :loading="loading" :items-per-page="15" class="elevation-1">
    ...
    <v-dialog v-model="dialogoNuevo" max-width="800" persistent>
      <v-card>
        <v-card-title class="text-h5">Nueva Venta / Factura</v-card-title>
        <v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn color="success" :loading="guardando" @click="guardarVenta" :disabled="!puedeGuardar || cargandoCatalogos">Crear Venta</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const facturas = ref([]); const clientes = ref([]); const vendedores = ref([]); const articulos = ref([]); const almacenes = ref([])
...
async function guardarVenta() {
  ...
  try {
    await apiClient.post('/api/v1/transacciones', {
      tipo: 'venta', entidad_cliente_id: nuevoDocumento.value.cliente_id,
    ...
    snackbar.value = { show: true, mensaje: 'Venta creada exitosamente', color: 'success' }
    cerrarDialogoNuevo()
